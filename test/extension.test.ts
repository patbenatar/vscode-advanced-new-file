...
const mockGetConfiguration = (
    config: {
        [key: string]: unknown;
    } = {showInformationMessages: true, expandBraces: false}
) => {
    return (name) => {
        switch (name) {
            case 'advancedNewFile':
                return {
                    get: (configName: string) => config[configName]
                };
            default:
                return {};
        }
...
describe('Advanced New File', () => {
    ...
    describe('directories', () => {
        ...
        context('with a gitignore file', () => {
            ...
            it('does not include gitignored directories', async () => {
                const result =
                    await AdvancedNewFile.directories(dummyProjectRoot);
                const relativePaths = result.map((r) => r.relative);
        ...
        context(
            'with a gitignore file in a directory above workspace root',
            () => {
                ...
                it('ignores as specified in both gitignore files', async () => {
                    const result =
                        await AdvancedNewFile.directories(testProjectRoot);
                    const relativePaths = result.map((r) => r.relative);

                    expect(relativePaths).not.to.include(
                        `${path.sep}nested-ignored`
                    );
        ...
        context('creating file', () => {
            context('file does not exist', () => {
                ...
                it('creates any nonexistent dirs in path', () => {
                    ...
                    expect(
                        fs.statSync(path.join(tmpDir, 'path/to')).isDirectory()
                    ).to.be.true;
                });
            ...
            context('file exists', () => {
                ...
                it('does not overwrite the file', () => {
                    ...
                    expect(
                        fs.readFileSync(existingFileDescriptor, {
                            encoding: 'utf8'
                        })
        ...
        context('creating folder', () => {
            context('folder does not exist', () => {
                ...
                it('creates a folder', () => {
                    ...
                    expect(
                        fs
                            .statSync(path.join(tmpDir, 'path/to/folder'))
                            .isDirectory()
                    ).to.be.true;
                });
            });
            ...
            context('folder with content exists', () => {
                ...
                const newFolderDescriptor =
                    path.join(tmpDir, 'folder') + path.sep;
    ...
    describe('openFile', () => {
        ...
        context('file can be opened successfully', () => {
            it('focuses the opened file', () => {
                ...
                return advancedNewFile.openFile('/path/to/file.ts').then(() => {
                    expect(showTextDocument).to.have.been.called.with(
                        textDocument
                    );
                });
            });
        ...
        context('file is a folder', () => {
            it('does not attempt to open it', () => {
                ...
                advancedNewFile
                    .openFile(path.join('path/to/folder/'))
                    .then(() => {
    ...
    describe('expandBraces', () => {
        context('expandBraces setting is true', () => {
            ...
            it('returns a single item array if there are no braces', () => {
                const fileArray =
                    advancedNewFile.expandBraces('test/file1.html');
    ...
    describe('currentEditorPath', () => {
        ...
        it('returns the abssolute path to file open in active editor', () => {
            ...
            expect(advancedNewFile.currentEditorPath()).to.eq(
                '/foo/bar/baz/bip'
            );
        });
    });
    ...
    describe('command integration tests', () => {
        ...
        it('creates and opens a file at given path', () => {
            ...
            const advancedNewFile = proxyquire<
            ...
            >('../src/extension', {
                vscode: {
                    ...
                    window: {
                        ...
                        showInputBox: () =>
                            Promise.resolve('/input/path/to/file.rb'),
                        showTextDocument
                    }
                },
                fs: {
        ...
        it('creates a folder at given path and shows information message', () => {
            ...
            const registerCommand = (_, commandFn: () => Promise<void>) =>
                (command = commandFn);
            ...
            >('../src/extension', {
                vscode: {
                    ...
                    window: {
                        ...
                        showInputBox: () =>
                            Promise.resolve('input/path/to/folder/'),
                        showInformationMessage,
                        showTextDocument
                    }
                },
            ...
            return command().then(() => {
                ...
                expect(showTextDocument).to.not.have.been.called.with(
                    textDocument
                );
                ...
                expect(fs.statSync(newFolderDescriptor).isDirectory()).to.be
                    .true;
            });
        });
